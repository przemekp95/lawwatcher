...
- `bash ops/run-structured-log-proof.sh` starts an isolated full-host Docker runtime, drives AI, replay, backfill and profile-subscription catch-up, and asserts stable structured log markers including `flow=ai`, `flow=document-ocr`, `flow=document-text-projection`, `flow=replay`, `flow=backfill` and `flow=profile-subscription`. Signed webhook delivery stays covered by the narrower `bash ops/run-signed-webhook-smoke.sh` proof.
