...
- `ops/compose/docker-compose.full-host.yml` is now a real expanded runtime: it has fresh green smoke with `api`, `portal`, `worker-ai`, `worker-projection`, `worker-notifications`, `worker-replay`, `worker-documents`, `sqlserver`, `rabbitmq`, `minio` and `ollama`.
...
- `bash ops/run-structured-log-proof.sh` starts an isolated full-host Docker runtime with signed HTTP webhook delivery, drives AI, replay, backfill, admin catch-up and signed-webhook flows, and asserts stable structured log markers including `flow=ai`, `flow=document-ocr`, `flow=document-text-projection`, `flow=replay`, `flow=backfill`, `flow=profile-subscription` or `flow=webhook-registration`, and `flow=signed-webhook`.
