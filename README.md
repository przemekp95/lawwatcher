...
Top-level source, operations, documentation, and tests live under the standard repo roots.
...
The checked-in Docker env examples keep the seeded notification webhook subscription, demo operator account, and
demo API-client tokens disabled by default. Enable `LAWWATCHER__SEEDDATA__ENABLEWEBHOOKSUBSCRIPTIONSEED=true`,
`LAWWATCHER__SEEDDATA__ENABLEDEFAULTOPERATORSEED=true`, or
`LAWWATCHER__SEEDDATA__ENABLEDEFAULTAPICLIENTSEED=true` only for local demo or smoke lanes; the supported
operational path does not depend on those seeds.

The checked-in Docker env examples now also include signed webhook delivery defaults through
`LAWWATCHER__WEBHOOKS__BACKEND=SignedHttp` and `LAWWATCHER__WEBHOOKS__SIGNINGSECRET=...`. Override the signing
secret before real deployments.
