...
- `publish-images.yml` is now the supported image-first publish lane: it pushes `ghcr.io/przemekp95/lawwatcher-*` images tagged with `sha-<commit>`, then runs an Ubuntu GHCR smoke through `bash ops/run-ghcr-image-smoke.sh` using those prebuilt images instead of `--build-local`.
- `docker-smoke.yml` stays the fast PR gate for Docker contract and build-local regression smoke; the heavier docker-first operational proofs (`write-path-nonblocking`, retention and structured logs) are available as shell scripts and are wired to manual `workflow_dispatch` CI only.
