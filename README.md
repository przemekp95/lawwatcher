...
- Seeded `LegalCorpus` acts now write real source artifacts into the selected document store, and `worker-ai` grounds `subjectType=act` tasks through `IDocumentStore` plus plain-text extraction before calling the local LLM. Completed task citations include both the act ELI and the stored artifact URI. That plain-text extraction path is transitional; the final product direction is real OCR over stored artifacts.
...
- `publish-images.yml` is now the supported image-first publish lane: it pushes `ghcr.io/przemekp95/lawwatcher-*` images tagged with long `sha-<full-commit>` tags, runs an Ubuntu GHCR smoke through `bash ops/run-ghcr-image-smoke.sh`, and exposes GHCR-backed `workflow_dispatch` ops proofs for `write-path-nonblocking`, retention and structured logs.
...
- `POST /v1/system/maintenance/retention` runs admin-only SQL retention cleanup for `published outbox`, `processed inbox`, the `event_feed` projection, optional `search_documents`, and terminal `ai_enrichment_tasks`. It accepts the same admin access as `/v1/system/messaging`; browser requests require antiforgery, bearer requests do not. `search_documents` cleanup remains opt-in through `searchDocumentsRetentionHours`, `ai_enrichment_tasks` cleanup is opt-in through `aiTasksRetentionHours`, and `documentArtifactsRetentionHours` is exposed truthfully but currently returns an explicit unsupported reason because the runtime does not yet track safe artifact ownership or expiry metadata for destructive document cleanup.
...
- `bash ops/run-act-ai-grounding-smoke.sh` starts an isolated `API + Worker.Ai` stack and verifies that an `act` task is grounded through stored source artifacts, plain-text extraction and local LLM citations.
...
- `bash ops/run-retention-smoke.sh` seeds an old terminal `ai_enrichment_tasks` row into SQL, executes `POST /v1/system/maintenance/retention` through the supported admin bearer path, and proves cleanup plus truthful `documentArtifactsRetentionApplied=false`.
- `bash ops/run-structured-log-proof.sh` starts an isolated full-host Docker runtime with signed HTTP webhook delivery, drives AI, replay, backfill, admin catch-up and signed-webhook flows, and asserts stable structured log markers including `flow=ai`, `flow=replay`, `flow=backfill`, `flow=profile-subscription` or `flow=webhook-registration`, and `flow=signed-webhook`.
